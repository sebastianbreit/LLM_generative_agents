...
a=sns.boxplot(x="m_i", y="max_score", hue="exp_success", data=df_trans, palette="Set3",ax=axes[0]).axhline(0.45)
a.set(xlabel="Memory item",ylabel='Max Similarity')
b=sns.boxplot(x="m_a", y="max_score", hue="exp_success", data=df_trans, palette="Set3",ax=axes[1]).axhline(0.45)
b.set(xlabel="Memory action",ylabel='Max Similarity')
for col in axes:
    col.set_ylim([0, 1])
...
a.set(xlabel="Query item",ylabel='Max Similarity',title='Abstract item Memory: abstract item')
b=sns.boxplot(x="q_a", y="max_score", hue="exp_success", data=df_trans, palette="Set3",ax=axes[0,1])
b.set(xlabel="Query action",ylabel='Max Similarity',title='Abstract item Memory: abstract item')
c=sns.boxplot(x="q_i", y="max_score", hue="exp_success", data=df_concrete, palette="Set3",ax=axes[1,0])
c.set(xlabel="Query item",ylabel='Max Similarity',title='Abstract item Memory: concrete item')
d=sns.boxplot(x="q_a", y="max_score", hue="exp_success", data=df_concrete, palette="Set3",ax=axes[1,1])
d.set(xlabel="Query action",ylabel='Max Similarity',title='Abstract item Memory: concrete item')
for row in axes:
    for col in row:
        col.set_ylim([0, 1])
        col.tick_params(labelrotation=45)
